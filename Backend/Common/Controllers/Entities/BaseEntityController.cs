...
public abstract class BaseEntityController<T> : ODataController where T : class, IEntity {
	...
	// GET By
	protected virtual ActionResult<SingleResult<T>> GetBy(System.Linq.Expressions.Expression<Func<T, bool>> function) {

		try {

			var record = DbSet.AsNoTracking().Where(function).AsQueryable();
	...
	// PUT By
	protected virtual async Task<ActionResult> PutByAsync(System.Linq.Expressions.Expression<Func<T, bool>> function, T item, int? user) {
		...
		try {
			...
			user ??= await GetUserAsync(user);
			var record = await DbSet.AsNoTracking().FirstOrDefaultAsync(function);
	...
	// PATCH By
	protected virtual async Task<IActionResult> PatchByAsync(System.Linq.Expressions.Expression<Func<T, bool>> function, Delta<T> item, int? user) {
		...
		try {
			...
			var record = await DbSet.FirstOrDefaultAsync(function);
	...
	// DELETE By
	protected virtual async Task<IActionResult> DeleteByAsync(System.Linq.Expressions.Expression<Func<T, bool>> function, int? user) {
